...
- For IPv4 packets, install flow rules matching inport, source and destination Ethernet addresses, and then output
...
class PathService(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if src_switch == dst_switch:
            ...
            self.logger.info("Install flow rules on %s", src_switch)

            if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                self.add_flow(self.switch_map[src_switch], 1, match, actions, msg.buffer_id)
            else:
                self.add_flow(self.switch_map[src_switch], 1, match, actions)        
        else:
